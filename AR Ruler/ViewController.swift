...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let toucheLocation = touches.first?.location(in: sceneView){
            let hitTestResults = sceneView.hitTest(toucheLocation, types: .featurePoint)
    ...
    func distanceCalculate(){
        let startDistance = dotNodeArray[0]
        let endDistance = dotNodeArray[1]
        
        print(startDistance)
        print(endDistance)
        
        let a = endDistance.position.x - startDistance.position.x
        let b = endDistance.position.y - startDistance.position.y
        let c = endDistance.position.z - startDistance.position.z
        
        let distance = sqrt(pow(a, 2) + pow(b, 2) + pow(c, 2)) * 100
       
        print(abs(distance)*100)
        updateText(text: "\(abs(distance))", atPosition: endDistance.position)
        //distance would never be negative
